...
abstract class AMPHTML_Tab_Abstract {
	...
	public function display_checkbox_field( $args ) {
		$id       = current( $args );
		$disabled = ( array_search( 'disabled', $args ) ) ? 'disabled' : '';
		...
		<?php if ( $this->options->get( $id, 'description' ) ): ?>
			<?php esc_html_e( $this->options->get( $id, 'description' ), 'amphtml' ) ?>
		<?php endif;
	}
	...
	public function display_multiple_select( $args ) {
		...
		<label for="<?php echo $id ?>">
			<select style="width: 30%" id="<?php echo $id ?>" name="<?php echo $name ?>" multiple>
				<?php ?>
