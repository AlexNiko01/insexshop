...
class AMPHTML_Options {
	...
	public function get_field_name( $id ) {
		return AMPHTML::TEXT_DOMAIN . '_' . $id;
	}
	...
	public function do_settings_fields( $page, $section ) {
		...
		$custom_fields = array( //todo remove by create own method for proper tab
			'logo_text',
			'ad_doubleclick_data_slot_top',
			'ad_doubleclick_data_slot_bottom',
			'ad_data_id_client_top',
			'ad_data_id_client_bottom',
			'ad_adsense_data_slot_top',
			'ad_adsense_data_slot_bottom',
	...
	public function custom_content( $object ) { ?>
		...
		<?php
		$content = ( get_post_meta( $object->ID, "amphtml-custom-content", true ) )
			? get_post_meta( $object->ID, "amphtml-custom-content", true ) : $object->post_content;
		wp_editor( $content, 'amphtml-custom-content' );
	}

	public function custom_title( $object ) {
		$title_value = ( get_post_meta( $object->ID, "amphtml-custom-title", true ) )
			? get_post_meta( $object->ID, "amphtml-custom-title", true ) : $object->post_title;
		?>
		<p>
			<label for="amphtml-custom-title"><?php _e( 'Title', 'amphtml' ) ?></label>
			<input type="text" name="amphtml-custom-title" size="30"
			       value="<?php echo $title_value; ?>" spellcheck="true" autocomplete="off">
	...
	public function display_exclude_options( $object ) {
		...
		<p>
			<label
				for="amphtml-metabox"><?php _e( "Disable AMP version of this page", 'amphtml' ) ?></label>
			<?php $checked = get_post_meta( $object->ID, "amphtml-exclude", true ); ?>
	...
	public function save_amphtml_meta_box( $post_id, $post, $update ) {
		...
		$default_options = array(
			'amphtml-exclude'          => '',
			'amphtml-custom-title'     => $post->post_title,
			'amphtml-override-title'   => '',
			'amphtml-custom-content'   => $post->post_content,
		...
		$options = array_intersect_key( $_POST, $default_options );
