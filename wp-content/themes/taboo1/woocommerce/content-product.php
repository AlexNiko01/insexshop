...
?>
<div class="product">
<?php /*<div <?php post_class( $classes ); ?>>*/ ?>
	...
	<?php// do_action( 'woocommerce_before_shop_loop_item' ); ?>
		...
		<div class="product-image-wrapper hover-effect-<?php if (has_post_thumbnail()) echo $hover; ?>">
			...
			<?php if (has_post_thumbnail()): ?>
				...
				?>
				<a href="<?php the_permalink(); ?>"  title="<?php the_title(); ?>" id="<?php echo etheme_get_image(false, 800, 1000, false); ?>" class="product-content-image <?php if($hover == 'tooltip'): ?>imageTooltip<?php endif; ?> <?php if ($hoverUrl != ''): ?>with-hover<?php endif ?>" data-images-list="<?php echo get_images_list($width, $height, $crop); ?>">
					<?php if ($hoverUrl != ''): ?>
						<img width="500" height="500" src="<?php echo $hoverImg['url']; ?>" class=" test show-image">
					<?php endif ?>
					<img width="500" height="500" <?php if($lazy_images): ?>data-<?php endif; ?>src="<?php echo $url; ?>" alt="<?php echo $alt_text; ?>" class="<?php if($lazy_images): ?>lazyOwl<?php endif; ?> hide-image">
				</a>
			<?php else: ?>
				<a href="<?php the_permalink(); ?>"  title="<?php the_title(); ?>" class="product-content-image <?php if($hover == 'tooltip'): ?>imageTooltip<?php endif; ?> <?php if ($hoverUrl != ''): ?>with-hover<?php endif ?>" data-images-list="<?php echo get_images_list($width, $height, $crop); ?>">
			...
			<?php if ($hover == 'description'): ?>
				<div class="product-mask">
					<div class="mask-text">
						...
						<p><a href="<?php the_permalink(); ?>"  title="<?php the_title(); ?>" class="read-more-link button"><?php _e('Read More', ETHEME_DOMAIN); ?></a></p>
					</div>
				</div>
			<?php endif ?>
		...
		<?php if (etheme_get_option('product_page_productname')): ?>
			<div class="product-name"><a href="<?php the_permalink(); ?>" title="<?php echo do_shortcode(get_the_excerpt()); ?>"><?php the_title(); ?></a></div>
		<?php endif ?>
