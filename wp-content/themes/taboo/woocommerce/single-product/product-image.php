					...
					<ul class="slides">
						...
						<li>
							<?php if ($lightbox_enabled && $zoom != 'disable'): ?>
								<a href="<?php echo $image_link; ?>" itemprop="image" title="<?php echo $image_title ?>" class="zoom-link woocommerce-main-image " rel="<?php echo $lightbox_rel; ?>">
									<i class="icon-resize-full"></i>
								</a>
							<?php endif ?>
							<?php if($zoom != 'disable' || $lightbox_enabled): ?><a href="<?php echo $image_link; ?>" title="<?php echo $image_title ?>" class="main-image " <?php if($zoom == 'disable' && $lightbox_enabled): ?>rel="<?php echo $lightbox_rel; ?>"<?php endif; ?> id="main-zoom-image"><?php endif; ?>
								<img src="<?php echo $image; ?>" alt="<?php echo $image_title ?>">
							...
							<?php if ($attachment_ids): ?>
								<?php foreach ($attachment_ids as $key => $value): ?>
									<li>
										<?php if ($lightbox_enabled && $zoom != 'disable'): ?>
											<a href="<?php echo etheme_get_image($value); ?>" class="zoom-link" title="Увеличить фотографию" rel="<?php echo $lightbox_rel; ?>"><i class="icon-resize-full"></i></a>
										<?php endif ?>
										<?php if($zoom != 'disable' || $lightbox_enabled): ?><a href="<?php echo etheme_get_image($value); ?>" title="Увеличить фотографию" <?php if($zoom == 'disable' && $lightbox_enabled): ?>rel="<?php echo $lightbox_rel; ?>"<?php endif; ?> class="main-image "><?php endif ?>
											<img src="<?php echo etheme_get_image($value, $mainImageWidth, $mainImageHeight, $mainImageCrop); ?>" alt="<?php echo $image_title ?>">
										<?php if($zoom != 'disable' || $lightbox_enabled): ?></a><?php endif ?>
									</li>
								<?php endforeach ?>						
							<?php endif ?>
									...
									<video controls="controls">
										<?php foreach($video_attachments as $video):  ?>
											...
											<?php if($video->post_mime_type == 'video/ogg' && !$video_ogg): $video_ogg = true; ?>
												...
												<a href="<?php echo $video->guid; ?>" title="Видео"><?php _e('Download Video', ETHEME_DOMAIN); ?></a>
											<?php endif; ?>
										<?php endforeach; ?>
									</video>
								</li>
						...
						<?php if ( !has_post_thumbnail()): ?>
							<?php if ($attachment_ids): ?>
								<?php foreach ($attachment_ids as $key => $value): ?>
									<li>
										<?php if ($lightbox_enabled && $zoom != 'disable'): ?>
											<a href="<?php echo etheme_get_image($value); ?>" title="Увеличить фотографию" class="zoom-link zoom" rel="<?php echo $lightbox_rel; ?>"><i class="icon-resize-full"></i></a>
										<?php endif ?>
										<a href="<?php echo etheme_get_image($value); ?>" title="Увеличить фотографию" <?php if($zoom == 'disable'): ?>rel="<?php echo $lightbox_rel; ?>"<?php endif; ?> class="main-image zoom"><img src="<?php echo etheme_get_image($value, $mainImageWidth, $mainImageHeight, $mainImageCrop); ?>" alt="<?php echo $image_title ?>"></a></li>
								<?php endforeach ?>						
							<?php endif ?>
						<?php endif; ?>
