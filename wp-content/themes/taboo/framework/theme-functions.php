...
if (!function_exists('etheme_logo')) {
    function etheme_logo() {
    	...
    	?>
        <?php if($logoimg): ?>
            <a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>"><img width="300" height="55" src="<?php echo $logoimg ?>" alt="<?php bloginfo('name'); ?>" /></a>
        <?php else: ?>
            <a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>"><img width="300" height="55" src="<?php echo PARENT_URL.'/images/logo.png'; ?>" alt="<?php bloginfo('name'); ?>"></a>
        <?php endif ;
    }
}
        ...
        ?>
            <ul class="links">
                <?php if ( is_user_logged_in() ) : ?>
                    <?php if(class_exists('Woocommerce')): ?> <li class="my-account-link"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="Перейти в аккаунт"><?php _e( 'Your Account', ETHEME_DOMAIN ); ?></a>
                    <div class="submenu-dropdown">
                        ...
                        <?php else: ?>
		                    <p class="a-center install-menu-info">Меню клиента</p>
                        <?php endif; ?>
                    </div>
                </li><?php endif; ?>
                        <li class="logout-link"><a href="<?php echo wp_logout_url(home_url()); ?>" title="Выйти из аккаунта"><?php _e( 'Logout', ETHEME_DOMAIN ); ?></a></li>
                <?php else : ?>
                    <?php 
                        $reg_id = etheme_tpl2id('et-registration.php'); 
                        $reg_url = get_permalink($reg_id);
                    ?>    
                    <?php if(class_exists('Woocommerce')): ?><li class="login-link"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="Войти в аккаунт"><?php _e( 'Sign In', ETHEME_DOMAIN ); ?></a></li><?php endif; ?>
                    <?php if(!empty($reg_id)): ?><li class="register-link"><a href="<?php echo $reg_url; ?>" title="Зарегистрироваться"><?php _e( 'Register', ETHEME_DOMAIN ); ?></a></li><?php endif; ?>
                <?php endif; ?>
            </ul>
        <?php
    }
	...
	function et_insert_fb_in_head() {
		...
		if ( !is_singular()) //if it is not a post or a page
			...
			if(!has_post_thumbnail( $post->ID )) { 
				$default_image = get_site_url().'/wp-content/themes/taboo/images/logo.png'; 
		...
		ob_start(); ?>
			<div style="background-color: #f5f5f5;width: 100%;-webkit-text-size-adjust: none;margin: 0;padding: 70px 0 70px 0;">
				<div style="-webkit-box-shadow: 0 0 0 3px rgba(0,0,0,0.025) ;box-shadow: 0 0 0 3px rgba(0,0,0,0.025);-webkit-border-radius: 6px;border-radius: 6px ;background-color: #fdfdfd;border: 1px solid #dcdcdc; padding:20px; margin:0 auto; width:500px; max-width:100%; color: #737373; font-family:Arial; font-size:14px; line-height:150%; text-align:left;">
			        <?php if($logoimg): ?>
			            <a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>" style="display:block; text-align:center;"><img style="max-width:100%;" src="<?php echo $logoimg ?>" alt="<?php bloginfo('name'); ?>" /></a>
			        <?php else: ?>
			            <a href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>" style="display:block; text-align:center;"><img style="max-width:100%;" src="<?php echo PARENT_URL.'/images/logo.png'; ?>" alt="<?php bloginfo('name'); ?>"></a>
			        <?php endif ; ?>
                                ...
                                while ($multislides->have_posts()) : $multislides->the_post();
                                    ...
                                    if(class_exists('Woocommerce')) {
                                        ...
                                        if (!$product->is_visible()) continue; 
                                            woocommerce_get_template_part( 'content', 'product' );
                                        if($sidebar_slider){
                                            if($_i%1 == 0 && $_i != $multislides->post_count) {
                                                echo '</div><!-- slide-item -->';
                                                echo '<div class="slide-item product-slide">';
                                            }
                                        } else {
...
if(!function_exists('etheme_breadcrumbs')) {
    function etheme_breadcrumbs() {
      ...
      if (is_front_page()) {
        ...
        echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . $homeLink . '"><span itemprop="name">' . $home . '</span></a></span> ' . $delimiter . ' ';
        ...
        if ( is_category() ) {
          ...
          echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . get_year_link(get_the_time('Y')) . '"><span itemprop="name">' . get_the_time('Y') . '</span></a></span> ' . $delimiter . ' ';
          echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '"><span itemprop="name">' . get_the_time('F') . '</span></a></span> ' . $delimiter . ' ';
          ...
          if ( get_post_type() == 'etheme_portfolio' ) {
            ...
            echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . $portfolioLink . '/"><span itemprop="name">' . __($post_type->labels->name, ETHEME_DOMAIN) . '</span></a></span>';
            if ($showCurrent == 1) echo ' ' . $delimiter . ' ' . $before . get_the_title() . $after;
          } elseif ( get_post_type() != 'post' ) {
            $post_type = get_post_type_object(get_post_type());
            $slug = $post_type->rewrite;
            echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . $homeLink . '/' . $slug['slug'] . '/"><span itemprop="name">' . $post_type->labels->singular_name . '</span></a></span>';
            if ($showCurrent == 1) echo ' ' . $delimiter . ' ' . $before . get_the_title() . $after;
          } else {
          ...
          echo '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . get_permalink($parent) . '"><span itemprop="name">' . $parent->post_title . '</span></a></span>';
          if ($showCurrent == 1) echo ' '  . $before . get_the_title() . $after;
          ...
          while ($parent_id) {
            ...
            $breadcrumbs[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemprop="item" href="' . get_permalink($page->ID) . '"><span itemprop="name">' . get_the_title($page->ID) . '</span></a></span>';
...
if(!function_exists('et_back_to_page')) {
    function et_back_to_page() {
        //echo '<a class="back-to" rel="nofollow" href="javascript: history.go(-1)"><span>‹</span>'.__('Return to Previous Page',ETHEME_DOMAIN).'</a>';
    }
}
        ...
        switch ($position) {
            case 'footer2':
                ...
                ?>
                    ...
                    <div class="row-fluid">
                        <div class="span3">
                            <span class="widget-title">About Company</h4>
                            <div>
            ...
            case 'footer9':
                ?>
                    <p><a href="<?php home_url(); ?>" title="<?php bloginfo('name'); ?>"><img src="<?php echo PARENT_URL.'/images/'; ?>logo-small.png" alt="<?php bloginfo('name'); ?>" class="logo-small"></a></p>
                <?php
                break;
            case 'footer10':
                ?>
...
class Et_Navigation extends Walker_Nav_Menu
{
    ...
    function start_el ( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $description = '';
        if(strpos($class_names,'image-item') !== false){$description = '<img src="'.do_shortcode($item->description).'" alt="'.$item->title.'" width="570" height="300"/>';}
...
if(!function_exists('generate_qr_code')) {
    function generate_qr_code($text='QR Code', $title = 'QR Code', $size = 128, $class = '', $self_link = false, $lightbox = false ) {
        ...
        if($lightbox) {
            $class .= ' qr-lighbox';
            $output = '<a href="'.$image.'" title="'.$title.'" rel="lightbox" class="'.$class.'">'.$title.'</a>';
        } else{
            $class .= ' qr-image';
            $output = '<img src="'.$image.'" title="'.$title.'" class="'.$class.'" />';
        }
