	...
	function et_brands($atts) {
		...
		if ($count > 0) {
		    ...
		    foreach ($terms as $term) {
				...
				if($thumbnail_id) {
					$output .= '<a href="' . get_term_link( $term ) . '" title="' . sprintf(__('View all products from %s', ETHEME_DOMAIN), $term->name) . '"><img src="' . etheme_get_image($thumbnail_id) . '" width="250" height="90" alt="' . $term->name . '"/></a>';		
				} else {
...
$a = shortcode_atts(array(
    ...
    $output .= '<img src="https://chart.apis.google.com/chart?cht='.$type.'&chd=t:'.$a['data'].'&chtt=&chl='.$a['labels'].'&chs=600x250&chf=bg,s,65432100&chco='.$a['data_colours'].'" />';
    $output .= '</div>';
    return $output;
}
...
function etheme_banner_shortcode($atts, $content) {
    ...
    return '<div class="banner '.$a['class'].' banner-font-'.$a['font_style'].' hover-'.$a['hover'].'" '.$onclick.'><div class="banner-content"><div class="banner-inner">'.do_shortcode($content).'</div></div><img src="'.$src.'" alt="'.$alt.'" height="300" width="570"/></div>';
}
// **********************************************************************// 
// ! Progress Bar
// **********************************************************************// 
...
function etheme_googlefont_shortcode($atts, $content = null) {
    ...
    if (!in_array($google_name, $registerd_fonts)) {
	    ...
	    ?>
	    <link rel='stylesheet'  href='https://fonts.googleapis.com/css?family=<?php echo $google_name; ?>' type='text/css' media='all' />
	    <?php
    }
                    ...
                    <div class="span6">
                        ...
                        <?php if (count($images)>0 && has_post_thumbnail()): ?>
                            <div class="post-images nav-type-small<?php if (count($images)>1): ?> images-slider<?php endif; ?>">
                                <ul class="slides">
                                     <li><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><img src="<?php echo $images[0]; ?>" alt="<?php the_title(); ?>" width="800" height="500"/></a></li>
                                </ul>
                                <div class="blog-mask">
                    ...
                    <div class="span6">
                        ...
                        <div class="post-info">
                            <span class="posted-on">
                                ...
                                <?php _e('в', ETHEME_DOMAIN) ?> 
                                <?php the_time(get_option('time_format')); ?>
                            </span> 
                            <span class="posted-by"> <?php _e('- Нимфоманка');?> <?php //the_author_posts_link(); ?></span>
                        </div>
