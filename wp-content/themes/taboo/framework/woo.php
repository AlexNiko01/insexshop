	...
	function et_product_brand_image() {
        ...
        if(count($terms)>0) {
			...
			<div class="product-brands">
				<div class="title"><span><?php _e('Product brand', ETHEME_DOMAIN) ?></span></div>
	        	<?php
			        	...
			        	if ($thumbnail_id) :
			                ...
			                ?>
			                	<a href="<?php echo get_term_link($brand); ?>" title="<?php echo $brand->name; ?>">
		                    		<img src="<?php echo $image; ?>" alt="<?php echo $brand->name; ?>" class="brand-image" />
			                	</a>
			                	<meta itemprop="brand" content="<?php echo $brand->name; ?>" />
			                <?php
			        	else :
			                ?>
			                	<h3><a href="<?php echo get_term_link($brand); ?>" title="<?php echo $brand->name; ?>"><span itemprop="brand"><?php echo $brand->name; ?></span></a></h3>
			                <?php
			        	endif;
			        }
	        	?>
...
if(!function_exists('et_brand_fileds')) {
	function et_brand_fileds() {
		...
		<div class="form-field">
			...
			<div id="brand_thumbnail" style="float:left;margin-right:10px;"><img src="<?php echo woocommerce_placeholder_img_src(); ?>" alt="<?php echo $brand->name; ?>" width="60px" height="60px" /></div>
...
if(!function_exists('et_edit_brand_fields')) {
    function et_edit_brand_fields( $term, $taxonomy ) {
    	...
    	<tr class="form-field">
    		...
    		<td>
    			<div id="brand_thumbnail" style="float:left;margin-right:10px;"><img src="<?php echo $image; ?>" alt="<?php echo $brand->name; ?>" width="60px" height="60px" /></div>
    			<div style="line-height:60px;">
		...
		if(is_user_logged_in()) {
			?>
				<div class="user-sidearea">
					...
					<br>
					<a href="<?php echo wp_logout_url(home_url()); ?>" title="Выйти из аккаунта"><?php _e('Logout', ETHEME_DOMAIN) ?></a>
				</div>
			<?php
		}
	}
		...
		?>
			<div id="loginModal" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true">
				<div>
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
						<span class="title"><span><?php _e('Login', ETHEME_DOMAIN); ?></span></span>
					</div>
					<div class="modal-body">
...
if(!function_exists('etheme_cart_modal')) {
	function etheme_cart_modal( $product_id = '' ) {
		...
		?>
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<span class="title"><span><?php _e('Cart', ETHEME_DOMAIN); ?></span></span>
			</div>
		<?php
            ...
            foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
                ...
                ?>
                    <div class="product-item">       
                        <a href="<?php echo get_permalink( $cart_item['product_id'] ); ?>" title="Корзина заказов" class="product-image">
                            <img src="<?php echo etheme_get_image(get_post_thumbnail_id($cart_item['product_id']), 100, 200, false); ?>" alt="Корзина заказов">
                        </a>
                        <?php 
        ...
        if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
          ...
          <?php
            ...
            ?>
                ...
                <a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="Корзина заказов" class="button left"><span><?php echo __('View Cart', ETHEME_DOMAIN); ?></span></a>  
                <a href="<?php echo $woocommerce->cart->get_checkout_url(); ?>" title="Оформить заказ" class="button filled active right "><span><?php echo __('Checkout', ETHEME_DOMAIN); ?></span></a>   
