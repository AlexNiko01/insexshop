jQuery(document).ready(function ($) {
    ...
    function et_sections() {
        $('.et_section').each(function () {
            ...
            videoTag.css({'width': $(window).width(),});
        });
    }
    ...
    function et_update_favicon() {
        ...
        var favicon = new Favico({animation: 'popFade', fontStyle: 'normal',});
    ...
    var mainNav = $('.fixed-menu-type1 .menu, .main-nav .menu-type1 .menu');
    mainNav.find('>li').hoverIntent({
        over: function () {
            var dropdown = $(this).find('> .nav-sublist-dropdown');
            var padding = 20;
            dropdown.fadeIn(100).css({display: 'list-item', listStyle: 'none'}).find('li').css({listStyle: 'none'});
            var columnsCount = dropdown.find('.container > ul > li').length;
            var dropdownWidth = dropdown.find('.container > ul > li').outerWidth();
            if (columnsCount > 1) {
    ...
    var fixedNav = $('.fixed-menu-type2 .menu');
    fixedNav.find('>li').hoverIntent({
        over: function () {
            var dropdown = $(this).find('> .nav-sublist-dropdown');
            dropdown.fadeIn(100).css({display: 'list-item', listStyle: 'none'}).find('li').css({listStyle: 'none'});
        }, out: function () {
    ...
    var mainNav2 = $('.main-nav .menu-type2 .menu');
    mainNav2.find('>li>a').click(function (e) {
        var animTime = 250;
        var timeOutTime = 0;
        ...
        if ($this.parent().hasClass('current')) {
            ...
            header.animate({'paddingBottom': 0}, animTime);
        } else {
            ...
            if ($this.parent().parent().find('.current').length > 0) {
                timeOutTime = animTime;
                header.animate({'paddingBottom': 0}, animTime);
            }
            $this.parent().parent().find('.current').removeClass('current').find('>div').slideUp(animTime);
            setTimeout(function () {
                $this.parent().addClass('current');
                header.animate({'paddingBottom': $this.next().height() + 20}, animTime);
                $this.next().slideDown(animTime);
            }, timeOutTime);
        }
    });
    var mainNav3 = $('.fixed-menu-type3 .menu, .main-nav .menu-type3 .menu');
    mainNav3.find('>li').hoverIntent({
        over: function () {
            var dropdown = $(this).find('> .nav-sublist-dropdown');
            dropdown.fadeIn(100).css({display: 'list-item', listStyle: 'none'}).find('li').css({listStyle: 'none'});
        }, out: function () {
    ...
    $portfolio.each(function () {
        ...
        portfolioGrid.isotope({itemSelector: '.portfolio-item'});
    ...
    $blog = $('.blog-masonry');
    $blog.isotope({itemSelector: '.post-grid'});
    $(window).smartresize(function () {
        $blog.isotope({itemSelector: '.post-grid'});
    });
    $(window).scroll(function () {
        if (!$('body').hasClass('fixNav-enabled')) {
            return false;
        }
    ...
    function ajaxProductLoad(url, blockId) {
        $.ajax({
            url: url, method: 'GET', timeout: 10000, dataType: 'text', success: function (data) {
    ...
    $('.etheme-simple-product').live('click', function (e) {
        ...
        if ($thisbutton.is('.etheme-simple-product, .product_type_downloadable, .product_type_virtual')) {
            ...
            var data = {quantity: $('input[name=quantity]').val()};
            modalWindow.eModal('showModal');
            $('body').trigger('adding_to_cart');
            $.ajax({
                url: formAction, data: data, method: 'POST', timeout: 10000, dataType: 'text', error: function (data) {
                    modalWindow.eModal('endLoading').eModal('addError', 'Error with ajax').eModal('addBtn', {
                        title: contBtn,
                    ...
                    modalWindow.eModal('endLoading').eModal('setTitle', productName).eModal('addImage', productImageSrc).eModal('addText', successfullyAdded).eModal('addBtn', {
                        title: contBtn,
                        href: 'javascript:void(0);',
                        cssClass: 'button small hidewindow',
                        hideOnClick: true
    ...
    $('.show-quickly').click(function () {
        ...
        $.ajax({
            ...
            error: function () {
                $.magnificPopup.open({
                    items: {src: '<div class="quick-view-popup mfp-with-anim"><div class="doubled-border">Error with AJAX request</div></div>'},
                    type: 'inline',
                    removalDelay: 500,
                    callbacks: {
                        beforeOpen: function () {
    ...
    $(document).on('click', '.price, .test-triggers', function () {
        // console.log('trigger works correctly');
    })
    ...
    leftTabs.each(function () {
        ...
        currTab.find('.tab-content').css({'minHeight': tabNewTitles.height()});
    });
    tabs.each(function () {
        var currTab = $(this);
        ...
        currTab.find('.tab-title, .tab-title-left').click(function (e) {
            ...
            if ($(this).hasClass('opened')) {
                ...
                if (currTab.hasClass('accordion') || $(window).width() < 767) {
                    ...
                    setTimeout(function () {
                        $('#content_' + tabId).addClass('tab-content').show();
                    }, 1);
                } else {
                    $('#content_' + tabId).show();
                }
    ...
    eForm.find('#submit').click(function (e) {
        ...
        if (errmsg) {
            ...
            $.ajax({
                url: url, method: 'GET', data: data, error: function (data) {
    ...
    progressBars.waypoint(function () {
        ...
        $(this).find('.progress-bar').each(function () {
            ...
            setTimeout(function () {
                el.find('div').animate({'width': width + '%'}, 400);
